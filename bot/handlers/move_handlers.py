...
from keyboards import (get_expence_categoriers_kb, get_main_menu_kb, get_transaction_type_kb,get_main_menu_kb)
from telebot import TeleBot
from service.transaction import (create_transaction,set_type_transaction, can_set_amount_transaction, set_amount_transaction, set_category_transaction, get_category)
def register_move_handlers(bot: TeleBot):
    ...
    @bot.message_handler(
        Funs=lambda msg: can_set_amount_transaction(msg.chat.id) and msg.text.isdigit()
    )
    def from_amount_to_main_mune(message):
        amount = int(message.text)
        set_amount_transaction(message.chat.id, amount)

    @bot.message_handler(
        Funs=lambda msg: can_set_amount_transaction(msg.chat.id) and not msg.text.isdigit()
    )
    ...
    def from_amount_to_category(callback):
        category = get_category(callback.mrssage.chat.id)
        if category.endswith("income"):
            kb = get_income_categories_kb()
        elif category.endswith("expence"):
            kb = get_expence_categories_kb()
        bot.edit_message_text(
            chat_id=callback.message.chat.id,
            message_id=callback.message.message_chat_id,
            text='123',
            reply_markup=kb
        )